...
bool Injector::manualMapDll(HANDLE hProc, BYTE* pSrcData, SIZE_T fileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport) {
	...
	if (reinterpret_cast<IMAGE_DOS_HEADER*>(pSrcData)->e_magic != 0x5A4D) { //"MZ"
		Logger::log(Logger::Type::Error, "Invalid file\n");
		return false;
	}
	...
	if (pOldFileHeader->Machine != CURRENT_ARCH) {
		Logger::log(Logger::Type::Error, "Invalid platform\n");
		return false;
	}
	...
	if (!pTargetBase) {
		Logger::log(Logger::Type::Error, "Target process memory allocation failed (ex) 0x%X\n", GetLastError());
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, pTargetBase, pSrcData, 0x1000, nullptr)) { //only first 0x1000 bytes for the header
		Logger::log(Logger::Type::Error, "Can't write file header 0x%X\n", GetLastError());
	...
	for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
		if (pSectionHeader->SizeOfRawData) {
			if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSrcData + pSectionHeader->PointerToRawData, pSectionHeader->SizeOfRawData, nullptr)) {
				Logger::log(Logger::Type::Error, "Can't map sections: 0x%x\n", GetLastError());
				VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
				return false;
			}
		}
	...
	if (!MappingDataAlloc) {
		Logger::log(Logger::Type::Error, "Target process mapping allocation failed (ex) 0x%X\n", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		return false;
	}

	if (!WriteProcessMemory(hProc, MappingDataAlloc, &data, sizeof(MANUAL_MAPPING_DATA), nullptr)) {
		Logger::log(Logger::Type::Error, "Can't write mapping 0x%X\n", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
	...
	if (!pShellcode) {
		Logger::log(Logger::Type::Error, "Memory shellcode allocation failed (ex) 0x%X\n", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		return false;
	}
	...
	if (!WriteProcessMemory(hProc, pShellcode, Shellcode, 0x1000, nullptr)) {
		Logger::log(Logger::Type::Error, "Can't write shellcode 0x%X\n", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
	...
	if (!hThread) {
		Logger::log(Logger::Type::Error, "Thread creation failed 0x%X\n", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE);
		return false;
	...
	while (!hCheck) {
		...
		if (exitcode != STILL_ACTIVE) {
			Logger::log(Logger::Type::Error, "Process crashed, exit code: %d\n", exitcode);
			return false;
		}
		...
		if (hCheck == (HINSTANCE)0x404040) {
			Logger::log(Logger::Type::Error, "Wrong mapping ptr\n");
			VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
			VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
			VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE);
			return false;
			...
			Logger::log(Logger::Type::Warning, "WARNING: Exception support failed!\n");
		}
	...
	if (emptyBuffer == nullptr) {
		Logger::log(Logger::Type::Error, "Unable to allocate memory\n");
		return false;
	}
	...
	if (ClearHeader) {
		if (!WriteProcessMemory(hProc, pTargetBase, emptyBuffer, 0x1000, nullptr)) {
			Logger::log(Logger::Type::Error, "WARNING!: Can't clear HEADER\n");
		}
	}
	//END CLEAR PE HEAD
			...
			if (pSectionHeader->Misc.VirtualSize) {
				if ((SEHExceptionSupport ? 0 : strcmp((char*)pSectionHeader->Name, ".pdata") == 0) ||
					...
					if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, emptyBuffer, pSectionHeader->Misc.VirtualSize, nullptr)) {
						Logger::log(Logger::Type::Error, "Can't clear section %s: 0x%x\n", pSectionHeader->Name, GetLastError());
					}
				}
			}
		}
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				...
				if (!VirtualProtectEx(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSectionHeader->Misc.VirtualSize, newP, &old)) {
					Logger::log(Logger::Type::Error, "FAIL: section %s not set as %lX\n", (char*)pSectionHeader->Name, newP);
				}
			}
		}
		DWORD old = 0;
	...
	if (!WriteProcessMemory(hProc, pShellcode, emptyBuffer, 0x1000, nullptr)) {
		Logger::log(Logger::Type::Warning, "WARNING: Can't clear shellcode\n");
	}
	if (!VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE)) {
		Logger::log(Logger::Type::Warning, "WARNING: can't release shell code memory\n");
	}
	if (!VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE)) {
		Logger::log(Logger::Type::Warning, "WARNING: can't release mapping data memory\n");
	}
...
void Injector::adjustPrivileges() {
	...
	// Открываем токен текущего процесса с правами на изменение привилегий
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
		priv.PrivilegeCount = 2;  // Устанавливаем количество привилегий в 2

		// Привилегия для отладки
		...
		// Привилегия для взятия на себя прав владения объектом
		priv.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;
...
bool Injector::injectDll(const std::wstring& dllPath, const std::string& processName) {
	...
	if (File.fail()) {
		Logger::log(Logger::Type::Error, "Opening the file failed\n");
		File.close();
		CloseHandle(hProc);
		return false;
	}
	...
	if (FileSize < 0x1000) {
		Logger::log(Logger::Type::Error, "Filesize invalid\n");
		File.close();
		CloseHandle(hProc);
		return false;
	}
	...
	if (!pSrcData) {
		Logger::log(Logger::Type::Error, "Can't allocate dll file\n");
		File.close();
		CloseHandle(hProc);
		return false;
	}
	...
	if (!manualMapDll(hProc, pSrcData, FileSize, false, false, true, false)) {
		delete[] pSrcData;
		CloseHandle(hProc);
		Logger::log(Logger::Type::Error, "Error while mapping\n");
		return false;
	}
	delete[] pSrcData;
